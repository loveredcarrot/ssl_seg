...
class LinkNetBaseWithDrop(nn.Module):
    """
    Generate model architecture
    """
    ...
    def __init__(self, n_classes=2):
        ...
        :type x_n: int
        """
        super(LinkNetBaseWithDrop, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, 7, 2, 3, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(3, 2, 1)
        ...
        self.encoder1 = Encoder(64, 64, 3, 1, 1)
        self.en_dropout1 = nn.Dropout(p=0.2)
        self.encoder2 = Encoder(64, 128, 3, 2, 1)
        self.en_dropout2 = nn.Dropout(p=0.2)
        self.encoder3 = Encoder(128, 256, 3, 2, 1)
        self.en_dropout3 = nn.Dropout(p=0.2)
        self.encoder4 = Encoder(256, 512, 3, 2, 1)

        self.decoder1 = Decoder(64, 64, 3, 1, 1, 0)
        # self.de_dropout1 = nn.Dropout(p=0.2)
        self.decoder2 = Decoder(128, 64, 3, 2, 1, 1)
        # self.de_dropout2 = nn.Dropout(p=0.2)
        self.decoder3 = Decoder(256, 128, 3, 2, 1, 1)
        # self.de_dropout3 = nn.Dropout(p=0.2)
        self.decoder4 = Decoder(512, 256, 3, 2, 1, 1)
    ...
    def forward(self, x):
        ...
        # Encoder blocks
        e1 = self.encoder1(x)
        e1 = self.en_dropout1(e1)
        e2 = self.encoder2(e1)
        e2 = self.en_dropout2(e2)
        e3 = self.encoder3(e2)
        e3 = self.en_dropout3(e3)
        e4 = self.encoder4(e3)

        # Decoder blocks
        # d4 = e3 + self.decoder4(e4)
        d4 = e3 + self.decoder4(e4)
        # d4 = self.de_dropout3(d4)
        d3 = e2 + self.decoder3(d4)
        # d3 = self.de_dropout2(d3)
        d2 = e1 + self.decoder2(d3)
        # d2 = self.de_dropout1(d2)
        d1 = x + self.decoder1(d2)
